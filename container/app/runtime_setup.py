...
async def ensure_setup_runtime_initialized(app: FastAPI) -> bool:
    ...
    async with lock:
        ...
        try:
            ...
            logger.warning("Failed to pre-warm HomeContext cache during in-process setup init", exc_info=True)
        ...
        try:
            ...
            logger.warning("Failed to load MCP servers from DB during in-process setup init", exc_info=True)
        ...
        if ddg_server is None:
            logger.info("Registering built-in DuckDuckGo MCP server during in-process setup init")
            ...
            if connected:
                try:
                    ...
                    logger.warning("Failed to auto-assign DuckDuckGo tools during in-process setup init", exc_info=True)
            else:
                logger.warning("DuckDuckGo MCP server registered but failed to connect during in-process setup init")
